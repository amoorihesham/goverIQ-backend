  ...
  return {
    async sendInvitation(
      ...
      return await withTx(async (tx) => {
        ...
        const pending = await tx.query.invitations.findFirst({
          where: and(
        ...
        if (pending) throw AppError.pendingInviteExists();
        ...
        if (role.isOwner) throw AppError.forbidden('Cannot invite to Owner role');
    ...
    async acceptInvitation(token: string, body?: { password?: string }) {
      ...
      return await withTx(async (tx) => {
        ...
        // Upsert membership
        const membership = await MemberRepository.upsertMembership(tx, userId, invitation.orgId, invitation.roleId);
    ...
    async listMembers(userId: string, orgId: string, cursor?: string, limit: number = 20) {
      // Verify membership
